    ...
    this.setState({
      ...
      windowListener: window.addEventListener('keydown', event => {
        ...
        if ([8, 46].indexOf(event.keyCode) !== -1 && selectedItems.length && deleteItems) {
          ...
          if (selectedItems.length === 1 && selectedItems[0]?.content?.type === 'textnode') return;
      ...
      action.selectionModels.forEach(model => {
        if (model.model instanceof NodeModel || model.model instanceof PointModel) {
          ...
          if (model.model instanceof NodeModel) {
            ...
            const movX = nextMovX;
            const movY = nextMovY;
            // get some multiple of factor near to the axis
            const multX = Math.floor(movX / fact);
            const multY = Math.floor(movY / fact);
            const getNextMove = (axis, k, m) => {
              const d1 = axis - k * m;
              const d2 = (k + 1) * m - axis;
              return d1 < d2 ? k * m : (k + 1) * m;
            };
            nextMovX = getNextMove(movX, fact, multX);
            nextMovY = getNextMove(movY, fact, multY);
          }

          model.model.x = nextMovX;
          model.model.y = nextMovY;
        }
      ...
      action.selectionModels.forEach(model => {
        ...
        if (model.model instanceof PointModel) {
          ...
          if (actionOutput.type === 'link-created') {
            diagramModel.removeLink(model.model.getLink());
          }
